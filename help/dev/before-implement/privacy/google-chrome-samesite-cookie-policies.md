...
考虑到 Facebook 在隐私和安全方面一直处于风尖浪口的位置，其他各大提供商，例如 Apple 以及现在的 Google，已经借助这一机会快速打造新的身份来赢得隐私和安全之役。 Apple领先Cookie政策，今年早些时候首次通过ITP 2.1以及最近的ITP 2.2宣布对其政策的更改。 在ITP 2.1中，Apple完全阻止第三方Cookie，并将浏览器中创建的Cookie仅保留七天。 在 ITP 2.2 中，Cookie 仅保留 1 天。 Google的公告远不及Apple那么激进，但这是向着相同目标迈出的第一步。 有关 Apple 政策的更多信息，请参阅 [Apple 智能防跟踪 (ITP) 2.x](/help/dev/before-implement/privacy/apple-itp-2x.md)。
...
在Adobe，我们始终希望支持最新的行业安全和隐私最佳实践。 我们非常高兴地宣布，[!DNL Target] 支持 Google 引入的新安全和隐私设置。
...
* **Target JavaScript库**：如果您在网站上使用at.js 1.*x*&#x200B;或at.js 2.*x*。
...
唯一会影响您的使用情形是当您通过at.js 1.*x*&#x200B;在[!DNL Target]中使用跨域跟踪功能时。 如果不转为 HTTPS（Google 要求转为 HTTPS），您会看到跨域的独特访客高峰，因为 Google 将丢弃我们使用的第三方 Cookie。 由于第三方 Cookie 将被丢弃，在用户从一个域导航到另一个域时，[!DNL Target] 无法为该用户提供一致的个性化体验。 第三方 Cookie 主要用于标识在您拥有的域之间导航的单个用户。

...
description: 使用 [!UICONTROL adobe.target.getOffer()] 函数及其选项 [!DNL Adobe Target] at.js库触发请求以获取 [!DNL Target] 选件。
title: 如何使用 [!UICONTROL adobe.target.getOffer()] 功能？
feature: at.js
...
| timeout | 数值 | 否 | 以毫秒为单位的超时时间。如果未指定，将使用 at.js 中的默认超时设置。<P>默认超时可以从 [!DNL Target] UI位于 [!UICONTROL 管理] > [!UICONTROL 实现]. |

## 示例

添加参数和 [!UICONTROL getOffer()] 和使用 [!UICONTROL applyOffer()] 对于成功处理：
...
添加参数和配置文件参数 [!UICONTROL getOffer()] 和使用 [!UICONTROL applyOffer()] 对于成功处理：
...
将自定义超时和自定义成功处理与 [!UICONTROL getOffer()]：

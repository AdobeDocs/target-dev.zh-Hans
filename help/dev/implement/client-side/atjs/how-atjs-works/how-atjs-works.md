...
请参阅[从at.js 1.x升级到at.js 2.x](/help/dev/implement/client-side/atjs/upgrading-from-atjs-1x-to-atjs-20.md)，以了解有关2.O中引入的与1.*x*&#x200B;的差异的更多信息。
...
| 步骤 | 描述 | 调用 | 描述 |
|--- |--- |--- |--- |
| 1 | 如果用户通过了身份验证，则调用会返回Experience Cloud ID (MCID)；另一调用会同步客户ID。 | 2 | at.js 库会同步加载，并隐藏文档正文。 |
| 3 | 将会发出全局 mbox 请求，其中包括已配置的所有参数，例如 MCID、SDID 和访客 ID（可选）。 | 4 | 轮廓脚本在执行后进入轮廓存储区。 存储区向受众库请求符合条件的受众（例如从Adobe Analytics、Audience Manager等共享的受众）。<br />客户属性会以批量过程发送到配置文件存储区。 |
| 5 | [!DNL Target] 根据 URL、mbox 参数和轮廓数据确定要返回给访客的活动和体验。 | 6 | 目标内容会发送回页面，其中可能包含其他个性化的轮廓值。<br />体验会在默认内容不发生闪烁的情况下尽快显示。 |
...
## at.js 如何渲染带有 HTML 内容的产品建议

在渲染带有 HTML 内容的产品建议时，at.js 会应用以下算法：

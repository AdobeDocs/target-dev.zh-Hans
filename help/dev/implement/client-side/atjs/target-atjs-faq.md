...
>[!NOTE]
>
>对于新访客，当访客API存在时，[!DNL Target]必须多次遍历线路以确保[!DNL Target]请求包含Experience Cloud访客ID数据。 对于回访访客，[!DNL Target]仅通过电线访问[!DNL Target]以检索个性化内容。
...
at.js实施使用单个库( at.js)，而mbox.js实施则实际使用两个库（ mbox.js和target.js）。 因此，更公平的比较方式是将 at.js 与 mbox.js *和* `target.js` 进行比较。 通过比较两个版本中的gzipped大小，at.js版本1.2为34 KB，mbox.js版本63为26.2 KB。 &quot;
...
## 我可以在单页应用程序中使用Target [!UICONTROL 可视化体验编辑器] (VEC)吗？
...
tt.omtrdc.net是Adobe EDGE网络的域名，用于接收[!DNL Target]的所有服务器调用。
...
只要targetGlobalSettings()函数([targetGlobalSettings](/help/dev/implement/client-side/atjs/atjs-functions/targetglobalsettings.md))中的`secureOnly`选项设置为true，与at.js启用的Adobe Edge网络之间的通信就只能通过HTTPS进行，否则，允许at.js根据页面协议在HTTP和HTTPS之间进行切换。
...
此外，对Adobe Edge Network的请求是公开的（因为它们设计为从访客的浏览器中发出），并且不包含可见的访客详细信息（它们仅包含访客ID）。 这些请求为访客提供体验，并包含有关访客应在页面上看到的内容的详细信息。
...
在at.js请求中，应该会看到值为“*”的`Access-Control-Allow-Origin`标头，因为它们是公共的，无需身份验证，并且需要通过JavaScript调用从任何域访问Adobe Edge Network。
...
at.js会试图避免长时间预先隐藏HTML BODY或其他DOM元素，但这具体取决于网络条件和活动设置。 at.js 会提供[设置](/help/dev/implement/client-side/atjs/atjs-functions/targetglobalsettings.md)，您可以使用这些设置来自定义主体隐藏 CSS 样式，这样您可以预先隐藏页面的某些部分，而不清空整个 HTML 主体。 预期的情况是这些部分包含必须进行“个性化”的 DOM 元素。

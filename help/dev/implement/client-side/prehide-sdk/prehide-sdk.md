...
></br>>请勿将`async`或`defer`添加到“预隐藏SDK”脚本标记。 需要同步执行，以便在浏览器开始布局页面之前注入隐藏规则。
...
当您从Flicker Manager UI下载`prehide.min.js`时，服务器将替换包中的三个占位符：

| 占位符 | 已替换为 | 如果未更换，则进行回退 |
| --- | --- | --- |
| `__FM_CLIENT_CODE__` | 您的客户端代码（例如，`"acmecorp"`） | 读取`window.PrehideConfig.org` |
| `__FM_TIMEOUT__` | 保护计时器持续时间（以毫秒为单位，例如`"3000"`） | `5000`毫秒 |
| `__FM_VERSION__` | SDK版本（例如，`"1.0.0"`） | `"0.0.0-dev"` |

如果您使用UI下载的捆绑包，则无需`PrehideConfig`块。 只需内联脚本。

### B：运行时`window.PrehideConfig` （手动集成）
...
| 字段 | 类型 | 必需 | 描述 |
| --- | --- | --- | --- |
| `org` | 字符串 | 是（除非已装入） | 您的客户客户端代码。 用作从中获取预隐藏规则的CDN URL的组织区段。 |

---
title: 受众定位
description: 受众可用于定向您的试验性和个性化活动。 [!DNL Adobe Target] 支持各种现成的强大受众定位功能。
...
受众可用于定向您的试验性和个性化活动。 [!DNL Adobe Target] 支持各种现成的强大受众定位功能。 以下属性可用于 [受众定位](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/create-audience.html)：

### [!DNL Target] 库

有关更多信息，请参阅 [[!DNL Target] 库](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/categories-audiences/target-library.html).&#x200B;AEM
* 反向链接来自Bing
* Chrome浏览器
* Firefox浏览器
* 反向链接来自Google
...
有关更多信息，请参阅 [网络](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/categories-audiences/network.html).
...
有关更多信息，请参阅 [自定义参数](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/categories-audiences/custom-parameters.html).

* 任意键/值对

### 操作系统

有关更多信息，请参阅 [操作系统](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/categories-audiences/operating-system.html).
...
有关更多信息，请参阅 [网页](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/categories-audiences/site-pages.html).
...
有关更多信息，请参阅 [浏览器](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/categories-audiences/browser.html).
...
有关更多信息，请参阅 [访客资料](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/categories-audiences/visitor-profile.html).

* 任何键/值对，将保留

### 流量源

有关更多信息，请参阅 [流量源](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/categories-audiences/traffic-sources.html).
...
从Node.js SDK v2.4.0和Java SDK v2.3.0开始，可将客户端提示发送到 [!DNL Target] via `getOffers()` 呼叫。 客户端提示应包含在 `request.context` 对象以及用户代理。
...
为了对基于地理位置的受众的设备上决策活动保持近乎零的延迟，Adobe建议您在调用中自己提供地理值， `getOffers`. 为此，请设置 `Geo` 中的对象 `Context` 请求的。 这意味着您的服务器将需要一种方法来确定每个最终用户的位置。 例如，您的服务器可能会使用您配置的服务执行IP到地理位置的查找。 某些托管提供商(如Google Cloud)通过每个提供商中的自定义标头提供此功能 `HttpServletRequest`.
...
但是，如果您无法在服务器上执行IP到地理位置的查找，但仍希望对执行设备端决策 `getOffers` 包含基于地理位置的受众的请求也支持此功能。 此方法的不利之处在于，它将使用远程IP到地理位置的查找，这会为每个查询添加延迟 `getOffers` 呼叫。 此延迟应低于远程延迟 `getOffers` 调用，因为它点击了位于服务器附近的CDN。 您必须 **仅限** 提供 `ipAddress` 中的字段 `Geo` 中的对象 `Context` ，以便SDK检索用户IP地址的地理位置。 如果除了 `ipAddress` 提供， [!DNL Target] SDK将不会获取地理位置元数据以进行解析。
...
| [期限](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/categories-audiences/time-frame.html) | 是 |
| [Experience Cloud受众](https://experienceleague.adobe.com/docs/target/using/integrate/mmp.html) (Adobe Audience Manager、Adobe Analytics和Adobe Experience Manager的受众 | 是 |

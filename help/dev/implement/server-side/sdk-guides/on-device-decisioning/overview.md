...
description: 了解如何使用 [!UICONTROL [!UICONTROL on-device decisioning]]以缓存您的 [!DNL Target] A/B和MVT活动可在您的服务器上执行内存中决策，并且几乎没有延迟。
title: 什么是设备上决策？
...
新一代 [!DNL Adobe Target] SDK现在提供 [!UICONTROL on-device decisioning]，可在服务器上缓存A/B和体验定位(XT)营销活动，并以接近零延迟的速度执行内存中决策，而不会阻止网络请求 [!DNL Adobe Target]的边缘网络。

[!DNL Adobe Target] 还提供了灵活性，通过实时服务器调用从实验性和ML驱动的个性化营销活动中提供最相关和最新的体验。 换言之，当性能最重要时，您可以选择利用 [!UICONTROL on-device decisioning]但是，当需要最相关和最新的体验时，可以改为进行服务器调用。 请参阅 [何时使用设备上决策和边缘决策](../../sdk-guides/on-device-decisioning/supported-features.md) 了解应使用其中一个而不使用另一个的用例。

>[!NOTE]
>
>设备上决策可用于客户端和服务器端实施。 本文介绍 [!UICONTROL on-device decisioning] 用于服务器端。 有关信息 [!UICONTROL on-device decisioning] 对于客户端，请参阅客户端实施文档 [此处](../../../client-side/atjs/on-device-decisioning/on-device-decisioning.md).

## 它是如何工作的？

安装和初始化 [!DNL Adobe Target] SDK与 [!UICONTROL on-device decisioning] 已启用，a *规则构件* 从距离您服务器最近的Akamai CDN下载并缓存到您服务器上的本地。 当请求检索 [!DNL Adobe Target] 体验是在服务器端应用程序内做出的，根据缓存的规则工件中编码的元数据在内存中做出有关返回哪个内容的决策，该工件定义了 [!UICONTROL on-device decisioning] A/B和XT活动。
...
## 如何配置要使用的客户 [!UICONTROL on-device decisioning]？

设备上决策适用于所有用户 [!DNL Adobe Target] 使用的客户 [!DNL Adobe Target] 服务器端SDK。 要启用此功能，请导航到 **[!UICONTROL Administration]** > **[!UICONTROL Implementation]** > **[!UICONTROL Account details]** 在 [!DNL Adobe Target] UI，并启用 **[!UICONTROL On-Device Decisioning]** 切换。

>[!NOTE]
>
>您必须具有管理员或审批者 *用户角色* 启用或禁用 [!UICONTROL On-Device Decisioning] 切换。
...
>[!NOTE]
>
>在初始化 [!DNL Adobe Target] 要使用的SDK [!UICONTROL on-device decisioning]. 规则工件将首先需要生成并传播到Akamai CDN，以便 [!UICONTROL on-device decisioning] 去工作。

### 包括所有现有 [!UICONTROL on-device decisioning] 工件切换中的符合条件的活动

切换此项 **日期** 当你想要活下来的时候 [!DNL Target] 符合条件的活动 [!UICONTROL on-device decisioning] 自动包含在工件中。

正在离开此切换 **关** 这意味着您需要重新创建和激活任何 [!UICONTROL on-device decisioning] 活动，以便将它们包含在生成的规则工件中。

## 我如何知道一个活动是 [!UICONTROL on-device decisioning] 有能力？

创建活动后，会创建一个名为的标签 **[!UICONTROL Decisioning Method]**&#x200B;在活动详细信息页面中可见，指示活动是否 [!UICONTROL on-device decisioning] 能力。

![替代图像](assets/asset-odd9.png)

您还可以看到以下所有活动： [!UICONTROL on-device decisioning] 功能位于 **[!UICONTROL Activities]** 通过添加列页面 **[!UICONTROL Decisioning Method]** 添加到活动列表。

![替代图像](assets/asset-odd7.png)

>[!NOTE]
>
>创建和激活具有以下特征的活动后： [!UICONTROL on-device decisioning] 该代码支持，则可能需要经过20分钟才能将其包含在生成并传播到Akamai CDN Po的规则工件中。

## 需要遵循的步骤概述是什么，才能确保 [!UICONTROL on-device decisioning] 通过以下方式成功交付活动 [!DNL Adobe Target]的服务器端SDK？
...
1. 创建和激活支持的活动类型 [!UICONTROL on-device decisioning]，并验证 **[!UICONTROL Decisioning Method]** 是 **[!UICONTROL On-Device Decisioning]** ，以了解该活动。
...
[!DNL Adobe Target] [!UICONTROL on-device decisioning] 支持几乎零延迟的内容交付。

这个7分钟的视频：

* 描述 [!UICONTROL on-device decisioning]，包括与其他方法的比较 [!DNL Target] 实现
* 演示如何启用 [!UICONTROL on-device decisioning] 在Target
* 检查已配置JSON内容的基于表单的编辑器活动示例
* 显示示例Node.JS SDK代码，其中包含进行以下操作所需的关键配置： [!UICONTROL on-device decisioning]
* 在浏览器中演示结果
